...
export function OutreachEmailDrawer() {
  ...
  return (
    <>
      <div className="fixed inset-0 z-[calc(var(--z-drawer)+1)] bg-black/30" onClick={closeDrawer} aria-hidden />

      <aside
        className="fixed bottom-0 top-[var(--topbar-height)] right-[var(--tasks-panel-width)] z-[calc(var(--z-drawer)+2)] flex w-[480px] flex-col border-l border-[var(--color-border)] bg-[var(--color-card)] shadow-2xl"
        aria-label={drawerTitle(mailType)}
      >
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-5 py-4">
          <h2 className="text-[var(--text-section-size)] font-bold">{drawerTitle(mailType)}</h2>
          <button
            type="button"
            onClick={closeDrawer}
